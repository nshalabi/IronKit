...
namespace IronPython.Runtime {
    ...
    internal static class CurrentVersion {
        ...
        static CurrentVersion() {
            // NADER: This is needed since IronPython now is not a separate DLL/Assembly
            var assembly = typeof(CurrentVersion).Assembly;
            var version = "DummyVersion";// assembly.GetName().Version;
            Major = 2;// version.Major;
            Minor = 7;// version.Minor;
            Micro = 10;// version.Build;
            Series = "0";// version.ToString(2);
            DisplayName = "DummyDisplayName";// $"IronPython {version.ToString(3)}";
            ...
            ReleaseLevel = "DummyReleaseLevel"; //split[split.Length - 2];
            ReleaseSerial = 0;// int.Parse(split[split.Length - 1]);
        }
    }
}
